...
namespace FixTests::Client
{
    ...
    void Application::run()
    {
        ...
        {
            try {
                if (const char action = queryAction(); action == '1') {
                    queryEnterOrder();
                } else if (action == '2') {
                    queryCancelOrder();
                } else if (action == '3') {
    ...
    void Application::queryEnterOrder()
    {
        ...
        switch (const int version = queryVersion(); version) {
        case 40:
  ...
  void Application::queryMarketDataRequest()
  {
    ...
    {
        FIX::OrdType ordType = queryOrdType();
        FIX43::NewOrderSingle newOrderSingle(queryClOrdID(), FIX::HandlInst('1'), querySide(), FIX::TransactTime(), ordType);
